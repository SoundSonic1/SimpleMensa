...
import com.soundsonic.simplemensa.data.cardcheck.Utils
import com.soundsonic.simplemensa.data.cardcheck.card.desfire.DesfireException
import com.soundsonic.simplemensa.data.cardcheck.card.desfire.DesfireFileSettings.ValueDesfireFileSettings
...
class IntercardReader : ICardReader {
    ...
    override fun readCard(card: DesfireProtocol?): ValueData? {
        ...
        val settings =
            Utils.selectAppFile(card, appId, fileId)
        return if (settings is ValueDesfireFileSettings) {
            ...
            try {
                ...
                Log.w(
                    TAG,
                    "Exception while trying to read value",
                    e
                )
                null
            }
